...
## Orden del listado — decisión tomada

`_list_regs()` llama `Paginate()` sin ningún `sort` — confirmado
empíricamente que el orden real es de inserción ascendente (más
antiguo primero), lo opuesto al Requisito 1.1. **Decisión: sí se
agrega un sort mínimo por modelo dentro de `_list_regs()`**, siguiendo
el mismo patrón ya existente en ese método para `sectionTitle`
(`switch ($this->_model): case 'project': ...`) — no es una
abstracción nueva, es extender un `switch` que ya existe con un caso
más:

```php
$sort = match ($this->_model) {
    'event' => '`id` DESC',
    default => '',
};

$this->data = $this->{$this->_model_camelized}->Paginate(
    $this->fullUrl(),
    ['conditions' => $conditions, 'sort' => $sort]
);
```

> **Verificar contra el framework real antes de implementar:** que
> `Paginate()` acepta una key `'sort'` igual que `Find()` (documentado
> en `dumbophp-models.md` para `Find()`, no confirmado para
> `Paginate()` específicamente). Si no la acepta, hay que encontrar el
> mecanismo real de ordenamiento de `Paginate()` — no asumido.
>
> Esto sí modifica `_list_regs()`, a diferencia de la restricción
> original del prompt anterior ("sin cambios adicionales ahí") — esa
> restricción quedó superada por esta decisión explícita. Verificar
> con cuidado que `project`/`group` (con `sort = ''`, el `default`)
> mantienen exactamente el comportamiento actual.
