import conf from '/dumbojs.conf.json' with { type: 'json' };
...
export class DumboDirective extends HTMLElement {
    ...
    setNodes(target) {
        const walker = document.createTreeWalker(target || this, NodeFilter.SHOW_ELEMENT);
        let node = null;
        let currReactives = [];
    ...
    async setTemplate() {
        ...
        if (this.constructor.templateUrl && this.constructor.templateUrl.length) {
            this.constructor.template = await this.loadTemplate(this.constructor.templateUrl);
        }
    ...
    async connectedCallback() {
        ...
        if (!this.hasAttribute('rendered')) {
            ...
            this.addEventListener(DmbEvents.inputChanged.listener, () => {
                let callbacks = this._onChangeActions;
                let callback = null;
    ...
    async loadTemplate(relativePath) {
        const url = new URL(relativePath, `${conf.baseUrl}${this.constructor.selector}/`);
