...
export class DmbSelect extends DumboDirective {
    ...
    init() {
        ...
        this.hasAttribute('searchable') && this._initSearchable(select);
    }

    _initSearchable(select) {
        if (this.querySelector('input.dmb-select-search')) return;

        const search = document.createElement('input');
        search.type = 'text';
        search.className = 'dmb-select-search';
        search.setAttribute('autocomplete', 'off');
        this.hasAttribute('label') && search.setAttribute('aria-label', this.getAttribute('label'));
        search.placeholder = 'Buscar...';

        search.addEventListener('input', () => {
            this._filterOptions(select, search.value);
        }, {passive: true});

        select.addEventListener('change', () => {
            search.value = '';
            this._filterOptions(select, '');
        }, {passive: true});

        this.insertBefore(search, select);
    }

    _filterOptions(select, term) {
        ...
        for (let i = 0, len = options.length; i < len; i++) {
            ...
            option.hidden = needle !== '' && !text.includes(needle) && !value.includes(needle);
        }
    }
