...
export class DmbPanel extends DumboDirective {
    ...
    attributeChangedCallback(attr, oldValue, newValue) {
        if (attr === 'source' && !!newValue && newValue.length) {
            this.url = newValue;
            this.loadExternalSource();
        }
    }

    #_buildHeaders(method = 'GET', headers = {}, body = null) {
        const options = {
            method,
            headers: null,
            mode: 'cors',
    ...
    loadExternalSource() {
        const options = this.#_buildHeaders('GET', {}, null);
        let wrapper = null;
        let sourceRequest = null;

        if (this.url && this.url.length) {
            sourceRequest = new Request(this.url, options);
            fetch(sourceRequest)
                .then(res => res.text())
                .then(data => {
